            ...
            {doctorsData.map((doctor) => (
              <div key={doctor.id} className="cardoctor">
                ...
                <div className="doctor-icons">
                  <FaStar
                    className={`icon ${favorites[doctor.id] ? 'active' : ''}`}
                    onClick={() => toggleFavorite(doctor.id)} // عند الضغط على النجمة
                  />
                  <div className="eye-icon" onClick={() => toggleEye(doctor.id)}>
                    <FaEye
                      className={`icon ${viewedDoctors[doctor.id] ? 'active' : ''}`}
                    />
                    <h3>{views[doctor.id] || 0}</h3> {/* عرض عدد المشاهدات */}
                  </div>
                </div>
